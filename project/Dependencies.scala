...
object Dependencies {
  def apply(): Seq[ModuleID] = Seq(
    scalaReflect, scalaMeta, scalatest, scalaMock, scalacheck, scalacheckShapeless, scalaTestContainers, s3mock,
    ...
    http4s, monix, fs2, scalaUri, betterFiles, sttp, awsJava, circe, parserCombinators
  ).flatten
  ...
  lazy val scribe: Seq[ModuleID] = Seq(
    "com.outr" %% "scribe" % "3.3.2" withSources() withJavadoc()
  )
  ...
  lazy val catsEffectTesting: Seq[ModuleID] = Seq(
    "com.codecommit" %% "cats-effect-testing-scalatest" % "0.5.1" % "test, it"
  )
  ...
  lazy val scalaUri: Seq[ModuleID] = Seq(
    "io.lemonlabs" %% "scala-uri" % "3.0.0" withSources() withJavadoc()
  )
